...
static uint32_t init_task_stack[800];	// 空闲任务堆栈
...
void init_main(void) {
    ...
    // 初始化任务
    task_init(&init_task, (uint32_t)init_task_entry, (uint32_t)&init_task_stack[800]);
    task_init(&first_task, 0, 0);
    write_tr(first_task.tss_sel);

    //int a = 3 / 0;
    // irq_enable_global();
    // first_task表示当前main的运行进程，他的stack会在首次调用swatich时初始化
    int count = 0;
    for (;;) {
