...
void bitmap_init (bitmap_t * bitmap, uint8_t * bits, int count, int init_bit) {
    bitmap->bit_count = count;      // 设置位数
    bitmap->bits = bits;            // 绑定位图存储空间

    int bytes = bitmap_byte_count(bitmap->bit_count);  // 计算需要多少字节
    kernel_memset(bitmap->bits, init_bit ? 0xFF: 0, bytes); // 初始化所有位
}
